...
file_name = "CD4_integrated_29012024.RData"
...
Idents(SO) <- "integrated_snn_res.0.4"
DefaultAssay(SO) <- "RNA"
...
Cell_Nr <- as.data.frame(summary(SO@meta.data$integrated_snn_res.0.4[cell_with_tcrs]))
Cell_Nr$totalCells <- summary(SO@meta.data$integrated_snn_res.0.4)
print(Cell_Nr)

write_csv(Cell_Nr, file ="/Users/emiliasere/Desktop/Emilia/scRNA/R_stuff/CD4/CD4_CellNr_TCRs.csv")

Cell_Nr_donor <- as.data.frame(summary(SO@meta.data$donor[cell_with_tcrs]))
Cell_Nr_donor$totalCells <- summary(SO@meta.data$donor)
print(Cell_Nr_donor)

write_csv(Cell_Nr_donor, file ="/Users/emiliasere/Desktop/Emilia/scRNA/R_stuff/CD4/CD4_CellNr_TCRs_donor.csv")
...
setwd("/Users/emiliasere/Desktop/Emilia/scRNA/R_Stuff/CD4")
save(SO.harmony, file = "CD4_harmony_OnlyTCRscells_18062024.RData")

###SELECTING RESOLUTION TO FURTHER ANALYZE#### Figures 1 a,b,c Author: Emilia Schneider Revueltas
...
CD4h_FAM_10 <- FindAllMarkers(SO.harmony, only.pos = TRUE, 
                           min.pct = 0.1, 
                           logfc.threshold = 0.25)

CD4h_FAM_10 <- CD4h_FAM %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
CD4h_FAM_10 <- as.factor(unique(CD4h_FAM_10$gene))

SO.harmony@meta.data$integrated_snn_res.0.9 <- "NULL"
